...
const Chat: React.FC = () => {
    ...
    const subscribeWebSocket = () => {
        if (stompClient && selectedConversationId !== -1) {
            const newSubscription = stompClient.subscribe(`/topic/${selectedConversationId}`, (response) => {
    ...
    const handleSendMessage = () => {
        if (stompClient) {
            stompClient.send(`/app/sendMessage/${selectedConversationId}`, {}, JSON.stringify({
